...
import { useKinds, APIProvider, useExport, useImport } from "./api";
...
function DatastoreAdminView({ project }: { project: string }) {
  const { data: kinds, error: kindsError } = useKinds();
      ...
      <div className="container mt-3">
        ...
        <Switch>
          <Route path="/kinds/:kind">
            {({ kind }) => <KindPage kind={kind} page={0} />}
          </Route>
          <Route path="/kinds/:kind/:page">
            {({ kind, page }) => (
              <KindPage kind={kind} page={parseInt(page, 10)} />
            )}
          </Route>
          <Route path="/query">
            <QueryPage />
